...
uint16_t sid_received_msg_length;
#define SID_RECEIVE_BUFF_SIZE 64
static uint8_t sid_buf[SID_RECEIVE_BUFF_SIZE] = {0};

LOG_MODULE_REGISTER(callbacks, CONFIG_SIDEWALK_LOG_LEVEL);
K_MSGQ_DEFINE(u_sid_received_msgq, sizeof(sid_received_msg_length), 10, 4);
				     ...
				     void *context)
{
	...
	//LOG_INF("received message(type: %d, link_mode: %d, id: %u, size %u, attr: %i)", (int)msg_desc->type,
	//	(int)msg_desc->link_mode, msg_desc->id, msg->size, msg_desc->msg_desc_attr.rx_attr.ack_requested);
	//LOG_HEXDUMP_INF((uint8_t *)msg->data, msg->size, "Message data: ");

    memcpy(sid_buf,(uint8_t *)msg->data,msg->size);
    sid_received_msg_length = msg->size;
    k_msgq_put(&u_sid_received_msgq, (void *)&sid_received_msg_length, K_NO_WAIT);
}
...
int u_sid_get_rx_buf(uint8_t *data_buff, size_t data_len)
{
    int ret = 0;

    //printk("%s\n",sid_buf);
    memcpy(data_buff,sid_buf,data_len);
    memset(sid_buf, 0, SID_RECEIVE_BUFF_SIZE); // reset buffer data 
