...
static void sidewalk_app_entry(void *ctx, void *data_packet, void *unused2)
{
	turn_data_packet *turn_packet = (turn_data_packet *)data_packet;
	ARG_UNUSED(unused2);
	...
	while (true) {
		...
		if (!k_msgq_get(&application_thread_msgq, &event, K_FOREVER)) {
			...
			case BUTTON_EVENT_SEND_HELLO:
				button_event_send_hello(application_ctx, turn_packet);
				break;
...
sid_error_t app_thread_init(app_ctx_t *ctx, turn_data_packet *data_packet)
{
	...
	(void)k_thread_create(&application_thread, application_thread_stack,
			      ...
			      ctx, data_packet, NULL, CONFIG_SIDEWALK_THREAD_PRIORITY, 0, K_NO_WAIT);
...
static void sidewalk_app_entry(void *ctx, void *unused1, void *unused2)
{
	ARG_UNUSED(unused1);
    ARG_UNUSED(unused2);
	app_ctx_t *application_ctx = (app_ctx_t *)ctx;
